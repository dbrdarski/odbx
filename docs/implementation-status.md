...
**value stores** are implemented. `init()` constructs the value stores. Write
orchestration, file persistence, Document/Revision assembly, transactional replay
and the public database API remain pending. Settled requirements stay in force
across all batches.
...
| §5, §7–8: String/Tuple/Record stores, canonical-reference Maps, subtree short-circuit, key/value decomposition, child-first discovery | Implemented in `src/stores.mjs`; references in `src/symbols.mjs`; store tests removed |
...
| §6–9: fork all counters, journal speculative mappings, failure consumes no IDs, stage publication | Counter fork and mapping rollback mechanisms exist in `src/stores.mjs`; write orchestration and Document/Revision staging pending |
...
| §9–11: rollback and truncate after partial writes, preserve committed state, block writes after failed recovery | Pending write orchestration and file adapter |
...
| §23.1: reuse across saves/documents, descendant traversal stops, provisional child reuse | Store tests removed; document saves pending |
| §23.2–3: counter rollback, retained failed canonical values, correct retry references | Store/write tests removed; write orchestration and Document/Revision behavior pending |
| §23.4: partial append failures, truncation, ID reuse and next successful transaction | Pending write orchestration and file adapter |
