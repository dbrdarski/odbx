...
test('Document definitions require random-id and type String references', () => {
  for (const input of ['<>', '<D\u0100>', '<S\u0100>', '<S\u0100E\u0101>', '<S\u0100S\u0101S\u0102>']) {
    failure(input);
  }
  for (const input of ['<', '<S', '<S\u0100S']) failure(input, true);
});
...
test('partial definitions and missing integer digits are incomplete at EOF', () => {
  for (const input of ['[', '[T', '[N', '[S', '[A\u0100', '{', '{A\u0100', '<S', '<S\u0100S', '(D\u0101O\u0100O\u0100F']) {
    failure(input, true);
  }
});
